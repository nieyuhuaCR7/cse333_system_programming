...
RO_FILE* ro_open(char* filename) {
  ...
  if (file->fd < 0) {
    // free the struct
    free(file);
    perror("getting file descripter wrong");
    return NULL;
  }
  ...
  if (file->buf == NULL) {
    close(file->fd);
    free(file);
    perror("allocating internal buffer wrong");
    return NULL;
  }
...
size_t flush_buffer(RO_FILE* file, char* out, int amount) {
  ...
  //    in the buffer.
  size_t num_to_flush;
  size_t remainder = file->buf_end - file->buf_index; // 内部缓冲区中剩余未刷新的字节数
  if (remainder <= amount) {
...
ssize_t fill_buffer(RO_FILE* file) {
  ...
  ssize_t bytes_read;
  // lseek(file->fd, file->buf_pos, SEEK_SET);
